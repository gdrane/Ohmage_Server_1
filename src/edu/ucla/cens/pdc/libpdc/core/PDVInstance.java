...
final public class PDVInstance implements CCNFilterListener {
	...
	public boolean handleInterest(Interest interest)
	{
		 ...
		 * 
		 * Ohmage Server commands:
		 * <app_id>/server/username/Eprivphone(hashedpassword)/<stream_id>/control/pull
		 *
		 * Manage commands:
		 * manage/list/apps
		...
		if(postfix.stringComponent(1).equals("server")) {
			// Get username
			String encrypted_IMEI = postfix.stringComponent(0);
			String encrypted_Hpasswd = postfix.stringComponent(1);
			// TODO(gdrane): Do Authentication and only then proceed
			// Also map to the particular data stream of the user like in here
			// there will be multiple similar named data streams like Gauresh->
			// Location, Derek->Location, check out the username and get appropriate
			// datastream object
			...
			return cmd.processCommand(ds, postfix, interest);
		}
